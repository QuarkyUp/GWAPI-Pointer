...
;~ Description: Turns rendering On if Off, Off if On using new hook
Func _ToggleRendering()
   If $mRendering Then
	  _NewRenderer_SetHook(True)
	  $mRendering = False
   Else
	  _NewRenderer_SetHook(False)
	  $mRendering = True
  EndIf
  WinSetState($mGWHwnd,"",$mRendering ? @SW_SHOW : @SW_HIDE)
EndFunc   ;==>ToggleRendering
