...
Global $mCollectItem    = DllStructCreate("PTR;DWORD;DWORD;DWORD")
...
Func Initialize($aGW = CharacterSelector(), $aChangeTitle = True, $aUseStringLog = False, $aUseEventSystem = True)
   ...
   If IsString($aGW) Then ; Charactername
	  $lWinList = WinList("[CLASS:ArenaNet_Dx_Window_Class]")
...
Func InitClient($aCharname = '')
   ...
   SetValue('RenderHook', '0x' & Hex($lTemp + 0x5D, 8))
   ;; TargetLog ;;
   ...
   SetValue('IncreaseAttributeFunction', '0x' & Hex($lTemp - 0x61, 8))
   SetValue('DecreaseAttributeFunction', '0x' & Hex($lTemp - 0x121, 8))
   ...
   DllStructSetData($mMaxAttributes, 5, $HEADER_MAX_ATTRIBUTES_CONST_5) ;# of attributes
   DllStructSetData($mMaxAttributes, 22, $HEADER_MAX_ATTRIBUTES_CONST_22) ;# of attributes
