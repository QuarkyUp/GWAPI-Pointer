...
Func GetItemPtrByAgentID($aAgentID)
   Return GetItemPtr(MemoryRead(GetAgentPtr($aAgentID) + 0xC8))
EndFunc   ;==>GetItemPtrByAgentID

;~ Description: Returns Itemptr by agentptr.
Func GetItemPtrByAgent($aAgentPtr)
 Return GetItemPtr(MemoryRead($aAgentPtr + 0xC8))
EndFunc
...
Func EquipItem($aItem)
   If IsPtr($aItem) <> 0 Then
	  Return SendPacket(0x8, $HEADER_ITEM_EQUIP, MemoryRead($aItem, 'long'))
   ElseIf IsDllStruct($aItem) <> 0 Then
	  Return SendPacket(0x8, $HEADER_ITEM_EQUIP, DllStructGetData($aItem, 'ID'))
   Else
	  Return SendPacket(0x8, $HEADER_ITEM_EQUIP, $aItem)
   EndIf
EndFunc   ;==>EquipItem
...
Func UseItem($aItem)
   If IsPtr($aItem) <> 0 Then
	  Return SendPacket(0x8, $HEADER_ITEM_USE, MemoryRead($aItem, 'long'))
   ElseIf IsDllStruct($aItem) <> 0 Then
	  Return SendPacket(0x8, $HEADER_ITEM_USE, DllStructGetData($aItem, 'ID'))
   Else
	  Return SendPacket(0x8, $HEADER_ITEM_USE, $aItem)
   EndIf
EndFunc   ;==>UseItem
...
Func DropItem($aItem, $aAmount = 0)
   If IsPtr($aItem) <> 0 Then
	  ...
	  Return SendPacket(0xC, $HEADER_ITEM_DROP, MemoryRead($aItem, 'long'), $aAmount)
   ElseIf IsDllStruct($aItem) <> 0 Then
	  If $aAmount = 0 Then $aAmount = DllStructGetData($aItem, 'Quantity')
	  Return SendPacket(0xC, $HEADER_ITEM_DROP, DllStructGetData($aItem, 'ID'), $aAmount)
   Else
	  If $aAmount = 0 Then $aAmount = MemoryRead(GetItemPtr($aItem) + 75, 'byte')
	  Return SendPacket(0xC, $HEADER_ITEM_DROP, $aItem, $aAmount)
   EndIf
EndFunc   ;==>DropItem
...
Func MoveItemEx($aItem, $aBag, $aSlot, $aAmount)
   ...
   Return SendPacket(0x14, $HEADER_ITEM_MOVE_EX, $lItemID, $lQuantity, $lBagID, $aSlot - 1)
EndFunc   ;==>MoveItemEx
...
Func UnequipItem($aEquipmentSlot, $aBag, $aSlot)
   ...
   EndIf
   Return SendPacket(0x10, $HEADER_ITEM_UNEQUIP, $aEquipmentSlot - 1, $lBagID, $aSlot - 1)
EndFunc   ;==>UnequipItem

;~ Description: Destroys an item.
Func DestroyItem($aItem)
   If IsPtr($aItem) <> 0 Then
	  Return SendPacket(0x8, $HEADER_ITEM_DESTROY, MemoryRead($aItem, 'long'))
   ElseIf IsDllStruct($aItem) <> 0 Then
	  Return SendPacket(0x8, $HEADER_ITEM_DESTROY, DllStructGetData($aItem, 'ID'))
   Else
	  Return SendPacket(0x8, $HEADER_ITEM_DESTROY, $aItem)
   EndIf
EndFunc   ;==>DestroyItem

;~ Description: Accepts unclaimed items after a mission.
Func AcceptAllItems()
   Return SendPacket(0x8, $HEADER_ITEMS_ACCEPT_UNCLAIMED, MemoryRead(GetBagPtr(7) + 8, 'long'))
...
Func PickUpItem($aAgentID)
   If IsPtr($aAgentID) <> 0 Then
	  Return SendPacket(0xC, $HEADER_ITEM_PICKUP, MemoryRead($aAgentID + 44, 'long'), 0)
   ElseIf IsDllStruct($aAgentID) <> 0 Then
	  Return SendPacket(0xC, $HEADER_ITEM_PICKUP, DllStructGetData($aAgentID, 'ID'), 0)
   Else
...
Func PickUpLoot($aMinSlots = 2, $aMe = GetAgentPtr(-2))
   ...
   Local $lAgentArray = MemoryReadAgentPtrStruct(1, 0x400)
...
Func CollectItem($nSrcModelId, $nCostCount, $nDstModelId)
	Local $pSrcItem = GetItemPtrByModelID($nSrcModelId, True)
...
Func ExchangeItems($aSrcModelID, $aCostCount, $aDstModelID, $aBagsOnly = True)
	Local $lStackPtr, $lStackBagPtr, $lStackSlot, $lItemPtr
	Local $lBagPtr
	Local $pDstItem = GetMerchantItemPtrByModelId($aDstModelID)
	If (Not $pDstItem) Then Return 0

	Local $lTotalItems = $aBagsOnly ? CountItemInBagsByModelID($aSrcModelID) : CountItemTotalByModelID($aSrcModelID)

	While $lTotalItems >= $aCostCount
		$lStackPtr = GetItemPtrByModelID($aSrcModelID, $aBagsOnly)
		$lStackBagPtr = MemoryRead($lStackPtr + 12, 'ptr')
		$lStackSlot = MemoryRead($lStackPtr + 78, 'byte') + 1

		; Test if stack is in storage
		If MemoryRead($lStackBagPtr + 8, 'long') - 1 > 4 Then
			$lSlot = OpenBackpackSlot()
			If IsArray($lSlot) Then
				MoveItem($lStackPtr, $lSlot[0], $lSlot[1])
				Sleep(GetPing() + Random(500, 750, 1))
			EndIf
		EndIf

		; If stack is too low, find more and move them
		If MemoryRead($lStackPtr + 75, 'byte') < $aCostCount Then
			For $i = 1 To ($aBagsOnly) ? 4 : 16
				$lBagPtr = GetBagPtr($i)
				For $j = 1 To MemoryRead($lBagPtr + 32, 'long')
					$lItemPtr = GetItemPtrBySlot($lBagPtr, $j)
					If $lItemPtr = 0 Then ContinueLoop
					If MemoryRead($lItemPtr + 44, 'long') = $aSrcModelID And $lItemPtr <> $lStackPtr Then
						MoveItem($lItemPtr, $lStackBagPtr, $lStackSlot)
						Sleep(GetPing() + Random(500, 750, 1))
						ExitLoop 2
					EndIf
				Next
		...
		Do
			DllStructSetData($mCollectItem, 2, MemoryRead($pDstItem))
			DllStructSetData($mCollectItem, 3, $aCostCount)
			DllStructSetData($mCollectItem, 4, MemoryRead($lStackPtr))

			Enqueue($mCollectItemPtr, 0x10)
			Sleep(GetPing())
		Until MemoryRead($lStackPtr + 75, 'byte') < $aCostCount

		$lTotalItems = $aBagsOnly ? CountItemInBagsByModelID($aSrcModelID) : CountItemTotalByModelID($aSrcModelID)
	WEnd
EndFunc
#EndRegion