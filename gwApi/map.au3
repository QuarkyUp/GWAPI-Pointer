...
Func WaitMapLoading($aMapID = 0, $aDeadlock = 30000)
   ...
   If GetMapLoading() = 1 Then
	  $mPartyArray = MemoryReadAgentPtrStruct(2, 0xDB, 1)
   Else
	  $CurrentMapID = $aMapID
...
Func WaitMapLoadingRenderingEnabled($aMapID = 0, $aDeadlock = 15000)
   ...
   WEnd
   If GetMapLoading() = 1 Then
	  $mPartyArray = MemoryReadAgentPtrStruct(3, 0xDB, 1, True)
   Else
	  $CurrentMapID = $aMapID
	  $mMaxPartySize = GetMaxPartySize(GetMapID())
   EndIf
...
;~ Descriptions: Returns time for instance.
Func GetInstanceTime()
   Local $lOffset[4] = [0, 0x18, 0x8, 0x1AC]
   Local $lTimer = MemoryReadPtr($mBasePointer, $lOffset)
   Return $lTimer[1]
EndFunc
...
;~    EndIf
   ...
   EndIf
   InitMapLoad()
   If Not SendPacket(0x18, $HEADER_MAP_TRAVEL, $aMapID, $lRegion, $aDistrict, $lLanguage, 0) Then Return
   Do
	  Sleep(250)
