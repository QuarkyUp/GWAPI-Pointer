...
Func MoveAggroing($MoveToX, $MoveToY, $SleepTime = 3600000, $aMyID = GetMyID(), $aMe = GetAgentPtr($aMyID))
   ...
   Do ;;;;;;;;;; Leader LOOP
	  PingSleep(200)
	  $Blocked += 1
	  Local $lAgentArray = MemoryReadAgentPtrStruct(1) ; all living
...
Func Death($aAgent = GetAgentPtr(-2), $aResign = True)
   ...
   If BitAND(MemoryRead($lAgentPtr + 312, 'long'), 0x0010) Then
	  Local $lPartyArray = MemoryReadAgentPtrStruct(3, 0xDB, 1, True)
...
;~    Local $lAgentArray = MemoryReadAgentPtrStruct(3, 0xDB, 1, True)
   Local $MoveX, $MoveY
...
Func GoPlayer($aAgent)
   If IsPtr($aAgent) <> 0 Then
	  Return SendPacket(0x8, $HEADER_AGENT_FOLLOW, MemoryRead($aAgent + 44, 'long'))
   ElseIf IsDllStruct($aAgent) <> 0 Then
	  Return SendPacket(0x8, $HEADER_AGENT_FOLLOW, DllStructGetData($aAgent, 'ID'))
   Else
...
Func GoSignpost($aAgent)
   If IsPtr($aAgent) <> 0 Then
	  Return SendPacket(0xC, $HEADER_SIGNPOST_RUN, MemoryRead($aAgent + 44, 'long'), 0)
   ElseIf IsDllStruct($aAgent) <> 0 Then
	  Return SendPacket(0xC, $HEADER_SIGNPOST_RUN, DllStructGetData($aAgent, 'ID'), 0)
   Else
	  Return SendPacket(0xC, $HEADER_SIGNPOST_RUN, ConvertID($aAgent), 0)
   EndIf
EndFunc   ;==>GoSignpost
