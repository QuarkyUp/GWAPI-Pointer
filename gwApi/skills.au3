...
Func SetSkillbarSkill($aSlot, $aSkillID, $aHeroNumber = 0)
   If $aHeroNumber = 0 Then
	  Return SendPacket(0x14, $HEADER_SKILL_CHANGE, GetMyID(), $aSlot - 1, $aSkillID, 0)
   Else
	  Local $lOffset[6] = [0, 0x18, 0x4C, 0x54, 0x24, 0x18 * ($aHeroNumber - 1)]
	  Local $lAgentID = MemoryReadPtr($mBasePointer, $lOffset)
	  Return SendPacket(0x14, $HEADER_SKILL_CHANGE, $lAgentID[1], $aSlot - 1, $aSkillID, 0)
   EndIf
...
Func LoadSkillBar($aSkill1 = 0, $aSkill2 = 0, $aSkill3 = 0, $aSkill4 = 0, $aSkill5 = 0, $aSkill6 = 0, $aSkill7 = 0, $aSkill8 = 0, $aHeroNumber = 0)
   If $aHeroNumber = 0 Then
	  Return SendPacket(0x2C, $HEADER_BUILD_LOAD, GetMyID(), 8, $aSkill1, $aSkill2, $aSkill3, $aSkill4, $aSkill5, $aSkill6, $aSkill7, $aSkill8)
   Else
	  Local $lOffset[6] = [0, 0x18, 0x4C, 0x54, 0x24, 0x18 * ($aHeroNumber - 1)]
	  Local $lAgentID = MemoryReadPtr($mBasePointer, $lOffset)
	  Return SendPacket(0x2C, $HEADER_BUILD_LOAD, $lAgentID[1], 8, $aSkill1, $aSkill2, $aSkill3, $aSkill4, $aSkill5, $aSkill6, $aSkill7, $aSkill8)
   EndIf
EndFunc   ;==>LoadSkillBar

;~ Description: Replaces skill in skillbarslot.
Func ReplaceSkill($aSkill, $aSlot, $aHeroNumber = 0)
   Return SendPacket(0x14, $HEADER_SKILL_CHANGE, GetHeroID($aHeroNumber), $aSlot - 1, $aSkill, 0)
...
;~ Description: Returns ptr to skillbar by HeroID.
...
Func UseSkillBySkillID($aSkillID, $aTargetType = 0, $aAgentID = -2)
   If $aTargetType = 0 Then
	  Return SendPacket(0x14, $HEADER_SKILL_USE_ALLY, $aSkillID, 0, ConvertID($aAgentID), 0)
   Else
	  Return SendPacket(0x14, $HEADER_SKILL_USE_FOE, $aSkillID, 0, ConvertID($aAgentID), 0)
   EndIf
		 ...
		 For $j = 0 To $lBuffCount - 1
			...
			Switch $lBuffAddress[1]
			   Case 263,244,242 ; $SKILLID_Protective_Bond, $SKILLID_Life_Attunement, $SKILLID_Balthazars_Spirit
				  SendPacket(0x8, $HEADER_STOP_MAINTAIN_ENCH, MemoryRead($lBuffAddress[0] + 8, 'long'))
			EndSwitch
		 Next
	  EndIf
   Next
...
Func ChangeSecondProfession($aProfession, $aHeroNumber = 0, $aHeroID = GetHeroID($aHeroNumber))
   Return SendPacket(0xC, $HEADER_CHANGE_SECONDARY, $aHeroID, $aProfession)
